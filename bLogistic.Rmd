...
# new <- tibble(dose = 0:5)
# new
# cbind(new, predictions(status.1, newdata = new)) 
# %>% 
  # select(dose, estimate)
```
\normalsize 
...
g + geom_point(data = brands, aes(x = age, y = proportion, colour = sex), inherit.aes = FALSE) -> g1
```
...
g + geom_point(
    data = brands,
    aes(x = age, y = proportion, colour = sex, size = Freq),
    inherit.aes = FALSE
  ) -> g2
```
\normalsize
