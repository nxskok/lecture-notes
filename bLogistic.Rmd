...
\normalsize
```{r bLogistic-19, size="footnotesize"}
...
## Improved plot
```{r bLogistic-33, fig.height=3.25, message=FALSE}
ggplot(miners, aes(x = Exposure, y = proportion,
                   colour = sev_ord)) + 
  geom_point() + geom_smooth(se = F)
```
...
```{r bLogistic-39, echo=F}
options(width = 70)
```

 
```{r bLogistic-40, size="small"}
