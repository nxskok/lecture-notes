...
or, make a graph (grouped bar chart):

```{r}
ggplot(dst, aes(x = gender, fill = agree)) + geom_bar(position = "dodge")
```
...
aside: Fisher's exact test, because both row and column totals are fixed (uses hypergeometric distribution)

```{r}
fisher.test(tab)
```

conclusion is the same, though P-value is (substantially) smaller. End of aside.
...
- First pick out each pair of groups:

```{r}
rats %>% filter(group != "Control") -> low_vs_high
rats %>% filter(group != "Lowjump") -> control_vs_high
rats %>% filter(group != "Highjump") -> control_vs_low
```

then compare the two groups in each one:

```{r}
t.test(density~group,data=low_vs_high)
t.test(density~group,data=control_vs_high)
t.test(density~group,data=control_vs_low)
```
