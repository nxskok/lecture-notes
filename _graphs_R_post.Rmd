...
read_delim(my_url, " ")
read_delim(my_url)
# read_table(my_url) # actually works, but not clear why
```
\normalsize
...
ggplot(athletes, aes(x = BMI)) + geom_histogram(bins = 8)
```
...
ggplot(athletes, aes(x = Sport, colour = Sex)) +
...
## BMI by sport and gender BMI by sport and gender

```{r graphs-R-10, fig.height=2.6}
