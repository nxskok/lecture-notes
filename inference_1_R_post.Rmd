...
```{r inference-1-R-post-4, echo=FALSE}
options(width=100)
```


```{r inference-1-R-post-5}
glimpse(jays)
```
\normalsize
...
```{r inference-1-R-post-6, fig.height=3.8}
...
```{r inference-1-R-post-11, fig.height=3}
ggplot(jays, aes(x = attendance)) + geom_histogram(bins = 6)
```
...
```{r inference-1-R-post-13}
kids
```

## Boxplots

```{r inference-1-R-post-14, fig.height=3.7}
...
```{r inference-1-R-post-15}
t.test(score ~ group, data = kids, alternative = "less")
```

## The pooled t-test 

```{r inference-1-R-post-16}
