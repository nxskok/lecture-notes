...
```{r inference-R-5, fig.height=3.8}
...
```{r inference-R-13, fig.height=3.7}
...
## The power curve
```{r inference-R-28, fig.height=3.8}
g
```
...
```{r inference-R-34, fig.height=3.8}
g
```
...
```{r inference-R-36, fig.height=3.8}
g
```
...
## But, look at a graph
```{r inference-R-45, fig.height=3.5}
...
Installation is a bit fiddly:
  ...
  - then install smmr: 
```{r inference-R-49, eval=F}
library(devtools)
install_github("nxskok/smmr")
```
- Then load it:
```{r inference-R-50, eval=F}
library(smmr)
```
...
```{r inference-R-70, fig.height=3.3}
...
```{r inference-R-71, fig.height=3.5}
...
```{r inference-R-74, fig.height=3.1}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-R-75, fig.height=3.1}
...
```{r inference-R-76, fig.height=3.3}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-R-77, fig.height=3.4}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-R-78, fig.height=3.4}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-R-81, fig.height=3.7}
ggplot(kids,aes(x=group,y=score))+geom_boxplot()
```
...
```{r inference-R-82, fig.height=3.2}
...
```{r inference-R-103, fig.height=3.5}
ggplot(rats,aes(y=density,x=fct_inorder(group)))+
geom_boxplot()
```
...
```{r inference-R-104, fig.height=3.5}
