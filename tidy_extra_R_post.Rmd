...
```{r tidy-extra-R-post-1}
library(tidyverse)
```

## The pig feed data again

```{r tidy-extra-R-post-2, message=F}
...
```{r tidy-extra-R-post-6, echo=F, message=F}
...
```{r tidy-extra-R-post-12, error=T}
pigs_longer %>% 
  group_by(feed) %>% 
...
```{r tidy-extra-R-post-13, error=T}
pigs_longer %>% 
  group_by(feed) %>% 
...
pigs_longer %>% 
  group_by(feed) %>% 
  summarize(r=list(quantile(weight))) %>% # this does 5-number summary 
  unnest_wider(r)
```
...
```{r tidy-extra-R-post-15, warning=FALSE}
pigs_longer %>% 
  group_by(feed) %>% 
...
```{r tidy-extra-R-post-16, echo=F, message=F}
