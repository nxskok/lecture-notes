...
[My source for this](https://www.researchgate.net/publication/265399426_Bootstrap_Methods_and_Permutation_Tests)
...
```{r bootstrap-2, include=F}
my_url <- "http://ritsokiguess.site/datafiles/irs.txt"
irs <- read_csv(my_url)
```


```{r bootstrap-3, fig.height=3.2}
...
cors
ggplot(cors, aes(x=my_cor))+geom_histogram(bins=20)
```
