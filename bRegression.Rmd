...
* To express uncertainty of this prediction:
  ...
  * *Prediction interval* expresses uncertainty about predicted ATST for a single new child aged 10 whose ATST not known. More uncertain. 


* Also do above for a child aged 5.

## The `marginaleffects` package 1/6 
...
## The `marginaleffects` package 2/6
...
## The `marginaleffects` package 3/6
...
* Age 10 closer to centre of data, so intervals are narrower than for age 5.
- Also true for representative ages, or all the ages. 
- See over (for CI for mean response).

## The `marginaleffects` package 4/6

```{r, fig.height=4}
plot_cap(model = sleep.1, condition = "age")
```


## The `marginaleffects` package 5/6

\tiny
```{r}
comparisons(model = sleep.1)
```
\normalsize

## The `marginaleffects` package 6/6

One for each row of data, but slope is the same for any obs, so can summarize:
...
For any age of child, a child that is one year older is expected to sleep for 14 minutes less.
...
* Or, *estimate* transformation, using Box-Cox method (over). 
...
ggplot(visits.1, aes(sample = .resid)) + 
...
ggplot(visits.3, aes(sample = .resid)) + 
  stat_qq() + stat_qq_line()
```
