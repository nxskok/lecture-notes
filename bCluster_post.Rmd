...
```{r bCluster-post-2, echo=F, fig.height=4}
set.seed(457299)
...
```{r bCluster-post-3, echo=F,fig.height=3.1}
...
```{r bCluster-post-4, echo=F,fig.height=3.4}
...
```{r bCluster-post-5, fig.height=3.2,echo=F}
...
```{r bCluster-post-6, echo=F,fig.height=3.5}
...
```{r bCluster-post-7, include=F}
options(width = 60)
```
 
\footnotesize
```{r bCluster-post-8, size="scriptsize",message=F}
...
```{r bCluster-post-9, size="footnotesize"}
number.d %>%
  select(-la) %>%
  as.dist() -> d
d
...
```{r bCluster-post-10, fig.height=4}
...
## Complete linkage
```{r bCluster-post-12, fig.height=4}
...
```{r bCluster-post-26, include=F}
options(width = 50)
```

     
```{r bCluster-post-27, size="footnotesize"}
...
```{r bCluster-post-28, size="scriptsize"}
...
```{r bCluster-post-35, size="footnotesize"}
vital %>% 
...
```{r bCluster-post-41, size="scriptsize"}
vital.km3$cluster
```
...
```{r bCluster-post-49, echo=FALSE}
set.seed(457298)
```

\small 
```{r bCluster-post-50, size="small"}
vital.s %>% 
...
\small
```{r bCluster-post-51, eval=FALSE,size="small"}
vital.s %>%
  select(-country) %>%
...
## The plot
```{r bCluster-post-66, fig.width=12, fig.height=10}
g
```
...
```{r bCluster-post-68, fig.height=4}
plot(ontario.hc)
...
## Seven clusters
```{r bCluster-post-69, fig.height=4 }
plot(ontario.hc)
