...
my_url <- "~/teaching/d29/notes/dogs.txt"
...
```{r}
summary(dogs.2)$multivariate.tests$times
```


## What there is here xxx
...
- If sphericity rejected, there are adjustments to the univariate P-values due to Huynh-Feldt and Greenhouse-Geisser. Huynh-Feldt better if responses not actually normal (safer). Or use multivariate test.

## Univariate tests xxx
...
## The multivariate test for interaction

\tiny

```{r}
summary(dogs.2)$multivariate.tests$`drug:times`
```
...
- The (appropriately adjusted) univariate test and the multivariate test should give similar results.
- Here, univariate with Huynh-Feldt P-value was 0.00073, multivariate was 0.0200, rather different but both significant.
...
- If the interaction had not been significant:
  ...
  - so look at univariate (better, especially if adjusted for sphericity) or multivariate tests of main effects in *this* model
...
- Huynh-Feldt adjusted P-value for interaction very similar, 0.094 (adjustment not really worth doing)
...
## Multivariate tests, all at once

\small

```{r bProfile-12, error=TRUE}
dogs.2
```

\normalsize 
 
Same conclusions as for univariate tests, though with bigger P-values.
...
## Re-fit without interaction xxx
...
## Reading the data xxx
Separated by *tabs*:  
```{r bProfile-16 }
# url <- "http://ritsokiguess.site/datafiles/exercise2.txt"
url <- "~/teaching/d29/notes/exercise.txt"
...
## Univariate results xxx

```{r bProfile-22, echo=F}
options(width=70)
```

## Multivariate very similar xxx
...
## Multivariate results (comparison) xxxx
