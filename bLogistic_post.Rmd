...
predictions(status.1) %>% 
  select(dose, predicted, conf.low, conf.high)
```
\normalsize 
...
```{r}
plot_cap(model = sev.1, condition = "Exposure", 
...
- Once again, `plot_cap` doesn't quite work
- so don't draw, edit, *then* make graph:

```{r}
plot_cap(brands.1, condition = c("age", "sex"), 
         type = "probs", draw = FALSE) %>% 
  rename(age = condition1, sex = condition2, 
         brand = group) %>% 
...
```{r}
plot_cap(brands.4, condition = c("age", "sex"), 
         type = "probs", draw = FALSE) %>% 
  rename(age = condition1, sex = condition2, 
         brand = group) %>% 
  ggplot(aes(x = age, y = predicted, colour = brand, 
