...
## Read in data
```{r bLogistic-post-17, size="footnotesize"}
my_url <- 
...
## The data
```{r bLogistic-post-18, size="footnotesize"}
sepsis
```
...
## Output part 1
```{r bLogistic-post-20, size="footnotesize"}
tidy(sepsis.1)
```
...
## Removing `malnut`
```{r bLogistic-post-21, size="footnotesize"}
...
\normalsize
```{r bLogistic-post-22, size="footnotesize"}
...
```{r bLogistic-post-23, size="small"}
...
\small
```{r bLogistic-post-24, ref.label="expo", echo=F}
```
\normalsize
...
\small
```{r bLogistic-post-31, fig.height=3.5, message=F}
...
## Improved plot
```{r bLogistic-post-36, fig.height=3.25, message=FALSE}
ggplot(miners, aes(x = Exposure, y = proportion,
                   colour = sev_ord)) + 
  geom_point() + geom_smooth(se = F)
```
...
```{r bLogistic-post-42, echo=F}
options(width = 70)
```

 
```{r bLogistic-post-43, size="small"}
...
```{r bLogistic-post-44, size="small"}
miners.pred %>%
...
```{r bLogistic-post-47, fig.height=3.8}
g
```
...
```{r bLogistic-post-52, error=TRUE}
...
```{r bLogistic-post-60 }
probs %>%
  pivot_longer(-(age:sex), names_to = "brand", values_to = "probability") -> probs.long
```
...
## The plot
```{r bLogistic-post-62, fig.height=2.8}
...
\footnotesize
```{r bLogistic-post-65, results="hide"}
b %>%
  mutate(sex = factor(sex)) %>%
...
```{r bLogistic-post-69, fig.height=3.2}
...
## The plot
```{r bLogistic-post-70, fig.height=3.7}
g
```
...
## The plot
```{r bLogistic-post-72, fig.height=3.7}
g
```
