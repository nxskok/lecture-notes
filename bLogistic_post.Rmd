...
predictions(sepsis.2, newdata = new) %>% 
  select(predicted, conf.low, conf.high, shock:bowelinf)
```
\normalsize
...
predictions(sepsis.2, newdata = new)
%>% 
  select(predicted, shock:age)
```


\footnotesize
```{r}
# predictions(sepsis.2, variables = "age") %>% select(predicted, shock:age)
```
\normalsize
...
## Making predictions
```{r bLogistic-54 }
brands %>% ungroup() %>% summarize(q = quantile(age, c(0, 0.25, 0.5, 0.75, 1)))
brands %>% ungroup() %>% count(sex)
new <- datagrid(age = c(24, 30, 33, 35, 38), sex = c("female", "male"), model = brands.1)
new
predictions(brands.1, newdata = new) %>%
