...
```{r bManova-10, size="footnotesize"}
...
- normal distribution of *each* response within each treatment group (actually *multivariate* normal with correlations between response variables)

  - assess with boxplots as usual (or normal quantile plots)

- each response has same spread across all treatment groups, and constant correlation between response variables across treatment groups
>>>>>>> 3a8258a1752a064efda41a13da7f96a255f55e71

  - assess with Box M test. This is very sensitive to unequal spread, so there is only a problem if P-value less than say 0.001. (`BoxM` in package `MVTests`.)
...
- Then pull out the numeric columns from dataframe, and (separately) the grouping column:
...
Make the plots (save to show on next page)
...
```{r}
peanuts %>% select(y:w) -> numbers 
summary(BoxM(numbers, peanuts$location))
summary(BoxM(numbers, peanuts$variety))
```
