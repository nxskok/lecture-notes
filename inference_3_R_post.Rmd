...
\footnotesize
```{r inference-3-R-post-3}
twogroups
```
\normalsize

## 95% CI (default)
```{r inference-3-R-post-4}
t.test(y ~ group, data = twogroups)
```

## 90% CI
```{r inference-3-R-post-5}
...
## Read in data 
```{r inference-3-R-post-7, message=FALSE}
...
## But, look at a graph
```{r inference-3-R-post-9, fig.height=3.5}
...
Installation is a bit fiddly:
  ...
  - then install smmr: 
```{r inference-3-R-post-13, eval=F}
library(devtools)
install_github("nxskok/smmr", force = TRUE)
```
- Then load it:
```{r inference-3-R-post-14, eval=F}
library(smmr)
```
...
```{r inference-3-R-post-25, eval=F}
lo = 200
hi = 220
while (hi - lo > 1) {
  try = (hi + lo) / 2
...
```{r inference-3-R-post-26, echo=F}
lo = 200
hi = 220
while (hi - lo > 1) {
  try = (hi + lo) / 2
...
```{r inference-3-R-post-29, echo=FALSE}
set.seed(457299)
```


```{r inference-3-R-post-30}
