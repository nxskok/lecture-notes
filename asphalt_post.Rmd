...
```{r asphalt-post-6, ref.label="logvisplot", echo=FALSE, message=FALSE}
...
```{r}
rut.1a <- lm(rut.depth ~ pct.a.surf + pct.a.base +
  voids + log(viscosity) + run, data = asphalt)
summary(rut.1a)
...
```{r}
rut.1a <- lm(rut.depth ~ pct.a.base + fines +
  voids + log(viscosity) + run, data = asphalt)
summary(rut.1a)
```
...
```{r asphalt-post-14, fig.height=7}
g
```
...
```{r asphalt-post-15, fig.height=7}
...
```{r asphalt-post-17, fig.height=7}
g3
```
...
```{r asphalt-post-43, fig.height=7}
g2
```
