...
## Check data 
```{r bRegression-post-3, size="footnotesize"}
sleep
summary(sleep)
```
...
```{r bRegression-post-8, size="footnotesize"}
tidy(sleep.1)
```

## also one-line summary of model:

```{r bRegression-post-9}
glance(sleep.1)
```
...
## Broom part 2
```{r bRegression-post-10, size="footnotesize",warning=F}
...
```{r bRegression-post-15, fig.height=3.0}
ggplot(sleep, aes(x = age, y = atst)) + geom_point() +
...
## Scatterplot 
```{r bRegression-post-16, fig.height=3.8}
...
```{r bRegression-post-21, size="small", fig.height=3.4}
...
## Some made-up data
```{r bRegression-post-22, message=F}
my_url <- "http://ritsokiguess.site/datafiles/madeup.csv"
# my_url <- "madeup.csv"
...
```{r bRegression-post-24, message=FALSE, fig.height=3.2 }
...
```{r bRegression-post-32, fig.height=3.5}
...
```{r bRegression-post-33, fig.height=2.8, message=FALSE}
...
```{r bRegression-post-34, echo=F, message=F, fig.height=4.3}
g
```
...
## Try 1
```{r bRegression-post-36, echo=F,fig.height=4.5}
...
\footnotesize
```{r bRegression-post-37, size="footnotesize"}
...
## Try 2
```{r bRegression-post-38, fig.height=3.5}
boxcox(timedrs + 1 ~ phyheal + menheal + stress,
  lambda = my.lambda,
  data = visits
)
...
```{r bRegression-post-41, fig.height=3.5}
...
## Normal quantile plot of residuals
```{r bRegression-post-42, fig.height=3.8}
...
```{r bRegression-post-43, fig.height=3.5, message=F}
