...
```{r functions-post-15, error=TRUE, warning=TRUE}
...
```{r functions-post-16}
sqrt_minus_value <- function(x, d = 1) {
  if (x-d < 0) stop("x less than d") else print("OK")
  # print("x should be greater than d")
  # stopifnot(x - d >= 0)
  # print(x)
  sqrt(x - d)
}
...
```{r functions-post-17, error=TRUE}
z <- sqrt_minus_value(8, 6)
z
```

- This should fail, and see how it does:
```{r functions-post-18, error=TRUE}
sqrt_minus_value(6, 8)
```
...
```{r functions-post-39, error=TRUE}
hotpo <- function(x) {
  stopifnot(round(x) == x)
  remainder <- x %% 2
  if (remainder == 1) {
...
```{r functions-post-46, results="hide"}
tibble(start = 1:7) %>%
...
```{r functions-post-47, results="hide"}
tibble(start = 1:7) %>%
  mutate(sequence = map(start, ~ hotpo_seq(.))) %>%
  mutate(
