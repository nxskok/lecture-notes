...
```{r inference-5-R-post-1, message=F}
...
```{r inference-5-R-post-2}
dst %>% count(gender, agree)
```


```{r inference-5-R-post-3}
...
```{r inference-5-R-post-6}
kids %>% count(group, score>m)
...
```{r inference-5-R-post-20, echo=F}
...
```{r inference-5-R-post-22, echo=F}
options(width=width)
```
...
```{r inference-5-R-post-23, fig.height=3.5}
ggplot(rats,aes(y=density,x=fct_inorder(group)))+
geom_boxplot()
```
...
```{r inference-5-R-post-24, fig.height=3.5}
...
```{r inference-5-R-post-32, eval=F}
library(PMCMRplus)
```
