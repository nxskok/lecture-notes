...
```{r inference-4a-R-post-1, echo=FALSE, message=FALSE}
...
```{r inference-4a-R-post-5, fig.height=4.5}
...
```{r inference-4a-R-post-6, fig.height=4}
...
```{r inference-4a-R-post-7, fig.height=4.5}
...
```{r inference-4a-R-post-8, echo=FALSE, message=FALSE}
jays <- read_csv("jays15-home.csv")
```


```{r inference-4a-R-post-9}
ggplot(jays, aes(x = attendance)) + geom_histogram(bins = 6)
```

## On a normal quantile plot

```{r inference-4a-R-post-10, fig.height=3.5}
...
```{r inference-4a-R-post-11, fig.height=4.5}
...
```{r inference-4a-R-post-12, fig.height=4.5}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```

## Normal data, small sample

```{r inference-4a-R-post-13, echo=F}
set.seed(457299)
```
...
```{r inference-4a-R-post-14, fig.height=4.5}
ggplot(d, aes(sample=x)) + stat_qq() + stat_qq_line()
```
...
```{r inference-4a-R-post-15, fig.height=4.5}
...
```{r inference-4a-R-post-16, fig.height=4.5}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-4a-R-post-17, fig.height=4.5}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
...
```{r inference-4a-R-post-18, fig.height=4.5}
ggplot(d,aes(sample=x))+stat_qq()+stat_qq_line()
```
