...
extra thought: a bootstrap sampling distribution of sample mean includes the sample size, so this needs to be close to normal.

The last one, with the very long tails and a sample size of 300:

```{r}
tibble(sim = 1:1000) %>% 
  rowwise() %>% 
  mutate(my_sample = list(sample(d$x, replace = TRUE))) %>% 
  mutate(my_mean = mean(my_sample)) %>% 
  ggplot(aes(sample = my_mean)) + stat_qq() + stat_qq_line()
```

this still has a long tail at the upper end, so a sample size of 300 is *not large enough*!