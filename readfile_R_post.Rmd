...
- from [this link](http://ritsokiguess.site/datafiles/test1.xlsx)
...
```{r readfile-R-post-4, eval=FALSE}
f <- file.choose()
```
...
```{r readfile-R-post-5, eval=FALSE}
mydata <- read_csv(f)
```
...
```{r readfile-R-post-8, echo=FALSE}
options(width = 80)
```
...
- [Example data:](http://ritsokiguess.site/datafiles/global.csv).
...
- Get the file yourself from [here](http://ritsokiguess.site/datafiles/coffee.txt).
...
```{r readfile-R-post-16}
drugs <- read_table("migraine.txt")
```

## The data

```{r readfile-R-post-17}
drugs
```
...
- Here is [my spreadsheet](http://ritsokiguess.site/datafiles/test2.xlsx) from before, but tarted up a bit:
...
```{r readfile-R-post-19, warning=F}
library(readxl)
...
```{r readfile-R-post-20}
tmp <- tempfile()
tmp
```

```{r readfile-R-post-21}
download.file("http://ritsokiguess.site/datafiles/test2.xlsx", tmp)
```
```{r readfile-R-post-22}
read_excel(tmp, sheet = "data")
```
