...
```{r inference-2-R-post-1, echo=F}
set.seed(457299)
```


```{r inference-2-R-post-2}
x <- rnorm(15, 8, 4)
x
```
...
```{r inference-2-R-post-5, echo=FALSE}
set.seed(457299)
```

```{r inference-2-R-post-6}
...
```{r inference-2-R-post-11}
ns=seq(10,100,10)
ns
```

- Calculate powers:
```{r inference-2-R-post-12}
...
## The power curve
```{r inference-2-R-post-15}
g
```

## Another way to do it:

```{r inference-2-R-post-16}
tibble(n=ns) %>% 
...
```{r inference-2-R-post-23, fig.height=3.8}
g
```
...
```{r inference-2-R-post-25, fig.height=3.8}
g
```

## Two-sample power

```{r inference-2-R-post-26, echo=FALSE}
