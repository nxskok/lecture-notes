...
##  Packages
```{r bFactor-post-1, eval=F, warning=F, message=F}
...
##  Scree plot
```{r bFactor-post-3, fig.height=3.5}
ggscreeplot(kids.pc)
```
...
```{r bFactor-post-10, message=FALSE}
...
```{r bFactor-post-11, fig.height=3.7}
g2
```
...
```{r bFactor-post-14, message=F, include=F}
...
\scriptsize
```{r bFactor-post-20, echo=-1}
# options(width = 60)
sort(bem.2$uniquenesses)
enframe(bem.2$uniquenesses) %>% arrange(value)
```
\normalsize
