...
```{r bProfile-post-4, echo=FALSE}
options(width = 70)
```


\footnotesize 
```{r bProfile-post-5, error=TRUE}
...
```{r bProfile-post-7, size="footnotesize"}
...
\small
```{r bProfile-post-10, fig.height=4}
# dogs.long
...
```{r bProfile-post-11, size="footnotesize", error=TRUE}
...
```{r bProfile-post-13, message=FALSE}
library(lme4)
# dogs.long
...
## The graph(s)
```{r bProfile-post-25, fig.height=5}
g
```
