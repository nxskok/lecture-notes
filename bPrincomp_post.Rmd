...
```{r bPrincomp-post-7,fig.height=3.8}
...
# test12 %>% select(across(where(is.numeric)))
test12_numbers
```
...
##   Scree plot
```{r bPrincomp-post-11, fig.height=3.5}
...
```{r bPrincomp-post-14,fig.height=3.5,}
...
##  The biplot
```{r bPrincomp-post-18,fig.height=4,echo=F}
g
```
...
\footnotesize
```{r bPrincomp-post-21, message=F}
my_url <- "http://ritsokiguess.site/datafiles/isocodes.csv"
iso <- read_csv(my_url)
iso
```
\normalsize
...
```{r bPrincomp-post-22, echo=FALSE}
options(width = 50)
```
...
track_num
track.pc <- princomp(track_num, cor = TRUE)
summary(track.pc)
```
\normalsize
...
```{r bPrincomp-post-24,fig.height=3.5,}
...
```{r bPrincomp-post-35, fig.height=5.5, warning=F}
g
```
...
```{r bPrincomp-post-37,fig.height=4,}
mat %>%
  as.matrix() %>%
...
```{r bPrincomp-post-38,fig.height=3.5}
ggscreeplot(mat.pc)
```
