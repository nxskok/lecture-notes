...
## Attendances and bootstrap sample
...
tibble(sim = 1:10000) %>% 
  ...
  mutate(sample = list(sample(jays$attendance, replace = TRUE))) %>% 
  mutate(my_mean = mean(sample)) %>% 
  ggplot(aes(x = my_mean)) + geom_histogram(bins = 15) -> g
```
\normalsize
...
## Doing both samples at once

- draw a bootstrap sample from both samples for each sim
- then look at distribution of difference between sample means (which is actually what counts)

```{r}
tibble(sim =1:1000) %>% 
  rowwise() %>% 
  mutate(treat_sample = list(sample(treats$score, replace = TRUE))) %>% 
  mutate(treat_mean = mean(treat_sample)) %>% 
  mutate(control_sample = list(sample(controls$score, replace = TRUE))) %>% 
  mutate(control_mean = mean(control_sample)) %>% 
  mutate(mean_diff = treat_mean - control_mean) %>% 
