...
## How bad is the normality? 

Bootstrap sampling distribution of sample mean (of the differences):
...
Skewed to the *left* (lowest values too low, highest not high enough). 

Bootstrap sample means can be very low, depending how often the low outlier shows up in bootstrap sample:
...
tibble(sim = 1:1000) %>% 
  ...
  mutate(my_sample = list(sample(pain$diff, replace = TRUE))) %>% 
  mutate(my_mean = mean(my_sample)) %>% 
  mutate(lows = sum(my_sample < (-5.5))) %>% 
  ggplot(aes(x = factor(lows), y = my_mean)) + geom_boxplot()
```

The more times that very low value shows up in the bootstrap sample, the lower the sample mean is (on average), so the one data value has an outsized influence on the sampling distribution of the sample mean.
