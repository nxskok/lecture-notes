...
```{r dates-and-times-post-1, eval=F}
library(tidyverse)
library(lubridate)
```
...
```{r dates-and-times-post-2, paged.print=T}
...
```{r dates-and-times-post-3, paged.print=T}
<<<<<<< HEAD
...
```{r dates-and-times-post-5, paged.print=T}
ddd
```
...
```{r dates-and-times-post-6, paged.print=T}
...
```{r dates-and-times-post-7, paged.print=T}
tibble(usdates = c("05/27/2012", "01/03/2016", "12/31/2015")) %>%
...
```{r dates-and-times-post-8, paged.print=F}
ddd
```
...
```{r dates-and-times-post-9, paged.print=T}
...
```{r dates-and-times-post-10, paged.print=F}
...
```{r dates-and-times-post-13, paged.print=T}
newdates
```
...
```{r dates-and-times-post-14, paged.print=T}
newdates %>%
  mutate(
<<<<<<< HEAD
    year = year(d),
...
minutes, seconds:
```{r dates-and-times-post-15, paged.print=F}
(dd <- tibble(text = c(
...
```{r dates-and-times-post-16, paged.print=F}
...
```{r dates-and-times-post-17, paged.print=T}
dd %>% 
...
- As you would expect: 
```{r dates-and-times-post-19, paged.print=F}
dd %>%
  select(-text) %>%
  mutate(
    h = hour(dt),
...
```{r dates-and-times-post-20, paged.print=T}
dd %>%
  select(dt) %>%
...
```{r dates-and-times-post-21, echo=F}
dd %>%
  mutate(oz = with_tz(dt, "Australia/Sydney")) %>%
  pull(oz)
```
...
```{r dates-and-times-post-24, paged.print=T}
