...
ggplot(jays, aes(x = attendance)) + 
  stat_qq() + stat_qq_line()
```

## Comments

- Distribution of attendances somewhat skewed to the right
...
ggplot(kids, aes(x = group, y = score)) + 
  stat_qq() + stat_qq_line() +
  facet_wrap(~ group)
```
...
- The sample size of $n = 12$ xxx is not large
